...
from .coordinates import add_coord, load_coords, save_coords
from .processor import MAP_TILE, process_image
...
class MapartBuilder:
    ...
    def launch(self, address: str, port: int):
        ...
        async def ready(ctx: ReadyContext):
            ...
            print("Commands: #build <image> [#build <image> --tile=N] | #coords | #help")
        ...
        async def player_message(ctx: PlayerMessageContext):
            ...
            try:
                if cmd == "build":
                    ...
                    await ctx.reply(ui.red("Unknown command. Use #help"))
    ...
    async def _handle_help(self, ctx):
        help_text = (
            ...
            "§e#build <image> [--x=X] [--z=Z] [--y=Y]§r\n"
            "  Default origin is (0, -1, 0) if --x/--z/--y omitted\n"
            "  §7Example: #build image.png --x=100 --z=200 --y=64§r\n"
            "§e#coords saveat <x> <z> [name]§r - Save location\n"
            "§e#coords list§r - List saved locations\n"
            "§e#coords clear§r - Clear saved locations\n"
            "§e#testblock <name>§r - Test if a block name works\n"
            "§e#help§r - Show this help"
        )
